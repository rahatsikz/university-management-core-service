...
import { IOfferedCourseSectionFilterOptions } from "./offeredCourseSection.interface";
...
const getAllFromDB = async(options: IPaginationOptions, filters:IOfferedCourseSectionFilterOptions)
:Promise<IGenericResponse<OfferedCourseSection[]>> => {
    ...
    const whereConditions:Prisma.OfferedCourseSectionWhereInput = andConditions.length > 0 ? { AND: andConditions } : {};
...
const getDataById = async(id: string):Promise<OfferedCourseSection | null> => {
...
const updateIntoDB = async(id: string, payload: Partial<OfferedCourseSection>):Promise<OfferedCourseSection> => {
